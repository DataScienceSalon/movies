...
As indicated by the kurtosis and skewness measures above, and as shown in `r kfigr::figr(label = "popularity", prefix = TRUE, link = TRUE, type="Figure")`, the distribution of popularity score was multimodal and slightly left skewed. 
```{r popularity}
edaUni$popularity$hist
```
`r kfigr::figr(label = "popularity", prefix = TRUE, link = TRUE, type="Figure")`: Distribution of movies by popularity


### Title Type
Feature films constituted `r edaUni$type$ctable$Proportion[2] * 100`% of the films in the sample. Since the focus of this study was theatrical releases, TV movies, which were included in the raw data were excluded from this analysis.
...
### Genre
The drama genre represented a plurality of the releases in the sample, followed by comedy action & adventure then mystery & suspense. The top four genres account for nearly `r round(sum(head(edaUni$genre$ctable %>% arrange(desc(Proportion)) %>% select(Proportion), 4)) * 100, -1)`% of the films in the sample.
```{r genre}
edaUni$genre$plot
```
...
### MPAA Rating
Rated R films accounted for over `r round(edaUni$mpaa$stats$Proportion[5] * 100, -1)`% of the releases, followed by PG and PG-13. Collectively, R, PG, and PG-13 rated films represent `r round(sum(head(edaUni$mpaa$ctable %>% arrange(desc(Proportion)) %>% select(Proportion), 3)) * 100, -1)`% of the films in the sample.
```{r mpaa}
edaUni$mpaa$plot
```
...
The plurality of features in the sample were released during the fall and summer months with over `r round(edaUni$season$ctable$Proportion[3] *100, -1)`% opening in the month of December alone. It was not evident that the sample reflects the population of movie releases in the United States. 
```{r season}
edaUni$season$plot
```
...
The plurality of features in the sample (`r sum(edaUni$month$ctable$Proportion[c(1,6,10,12)])*100`%) were released during the months of January, June, October and December.  It was not evident that the sample reflects the population of movie releases in the United States. 
```{r month}
edaUni$month$plot
```
...
`r kfigr::figr(label = "imdb_rating_ctable", prefix = TRUE, link = TRUE, type="Table")`: Releases by IMDB rating
```{r imdb_rating_ctable}
...
As indicated in `r kfigr::figr(label = "best_actor", prefix = TRUE, link = TRUE, type="Figure")`, the percentages of films with best director, actor and actress oscars were `r edaUni$bestDirWin$ctable$Proportion[2] * 100`%, `r edaUni$bestActorWin$ctable$Proportion[2] * 100`%,  and `r edaUni$bestActressWin$ctable$Proportion[2] * 100`%, respectively.  Again, these proportions indicate that oscar awards would not be a good predictor of movie popularity. The bivariate analysis will explore this further.
...
### Studio Votes
This was a derived variable intended to reflect the popularity of a given studio. `r kfigr::figr(label = "studio_votes_ctable", prefix = TRUE, link = TRUE, type="Table")` provides the summary statistics.  

`r kfigr::figr(label = "studio_votes_ctable", prefix = TRUE, link = TRUE, type="Table")`: Total IMDB votes by studio summary statistics
```{r studio_votes_ctable}
...
### Studio Experience
This was a derived variable intended to reflect the relative experience of a given studio. `r kfigr::figr(label = "studio_experience_ctable", prefix = TRUE, link = TRUE, type="Table")` provides the summary statistics.
`r kfigr::figr(label = "studio_experience_ctable", prefix = TRUE, link = TRUE, type="Table")`: Studio experience summary statistics
```{r studio_experience_ctable}
...
### Director Experience
This was a derived variable intended to reflect the relative experience of a given director. `r kfigr::figr(label = "director_experience_ctable", prefix = TRUE, link = TRUE, type="Table")` provides the summary statistics.
`r kfigr::figr(label = "director_experience_ctable", prefix = TRUE, link = TRUE, type="Table")`: Director experience summary statistics
```{r director_experience_ctable}
...
This was a derived variable intended to reflect the relative experience of a given cast for a film. `r kfigr::figr(label = "cast_experience_ctable", prefix = TRUE, link = TRUE, type="Table")` provides the summary statistics.
`r kfigr::figr(label = "cast_experience_ctable", prefix = TRUE, link = TRUE, type="Table")`: Film Cast experience summary statistics
```{r cast_experience_ctable}
knitr::kable(edaUni$castExperience$stats, digits = 2) %>%  
  kableExtra::kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = T, position = "center")
```

The top 10 films in terms of relative cast experience
...
```
Here, the relationships between the response variable, movie popularity, and the explanatory variables listed in `r kfigr::figr(label = "predictors", prefix = TRUE, link = TRUE, type="Table")` are examined. 
...
### Title Type
As shown in `r kfigr::figr(label = "bi_type", prefix = TRUE, link = TRUE, type="Figure")`, there appears to be a significant difference in mean popularity score at each level of the title type variable.

```{r bi_type}
edaBi$type$plot
```
`r kfigr::figr(label = "bi_type", prefix = TRUE, link = TRUE, type="Figure")`: Popularity by title type boxplot

`r kfigr::figr(label = "bi_type_mod", prefix = TRUE, link = TRUE, type="Table")`: Linear model of popularity by title type
```{r bi_type_mod}
knitr::kable(edaBi$type$mod) %>%  
  kableExtra::kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = T, position = "center")
```
`r kfigr::figr(label = "bi_type_aov", prefix = TRUE, link = TRUE, type="Table")`: Analysis of variance for popularity by title type
```{r bi_type_aov}
knitr::kable(edaBi$type$aov) %>%  
  kableExtra::kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = T, position = "center")
```
`r kfigr::figr(label = "bi_type_mod", prefix = TRUE, link = TRUE, type="Table")` indicates that the difference in mean popularity scores among documentaries, feature-films, and TV movies was significant. The analysis of variance in `r kfigr::figr(label = "bi_type_aov", prefix = TRUE, link = TRUE, type="Table")` shows that the amount of variance explained by the title type was significant, $F$(`r edaBi$type$aov$df[1]`, `r edaBi$type$aov$df[2]`) = `r round(edaBi$type$aov$statistic[1],0)`, $p$ = `r round(edaBi$type$aov$p.value[1], 3)`. As such, title type was considered a significant predictor of movie popularity.
