...
```{r preprocess}
dataSets  <- movies::preprocess(movies, boxOffice)
mdb <- dataSets$mdb
mdbBox <- dataSets$mdbBox
```
...
## Univariate Analysis
```{r univariate}
edaUni <- movies::univariate(mdb, mdbBox)  
```
...
#### Studio
The data included films from `r length(unique(mdb$studio))` studios. Data with respect to the number of films in the sample per studio are captured in the studio experience variable below.

#### Director
The work of `r length(unique(mdb$director))` directors was included in the sample provided for this project. Data with respect to the number of films in the sample per director are captured in the director experience variable below.
...
## Bivariate Analysis
```{r process, fig.height=4}
dataSets <- process(mdb, mdbBox)
mdb <- dataSets$mdb
mdbBox <- dataSets$mdbBox
```
